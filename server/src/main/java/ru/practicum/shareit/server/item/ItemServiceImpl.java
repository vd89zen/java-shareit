...
import ru.practicum.shareit.server.request.RequestItemService;
...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    @Transactional
    public Item createItem(Long ownerId, NewItemDto newItemDto) {
        log.info("Добавление пользователем {} новой вещи: {}.", ownerId, newItemDto);

        Item newItem = ItemMapper.toItem(newItemDto);
        newItem.setOwner(userService.getUserById(ownerId));

        if (newItemDto.getRequestId() != null) {
            newItem.setRequest(requestItemService.getRequestItemById(newItemDto.getRequestId()));
        }
    ...
    public Item getItemById(Long itemId) {
        log.info("Получаем вещь ID {}.", itemId);
        return getItemOrThrow(itemId);
    }

    @Override
    public List<Item> getAllItemForOwner(Long ownerId) {
    ...
    @Override
    @Transactional
    public Item updateItemById(Long ownerId, Long itemId, ItemUpdateDto itemUpdateDto) {
    ...
    @Override
    @Transactional
    public Comment commentItemById(Long userId, Long itemId, NewCommentDto newCommentDto) {
